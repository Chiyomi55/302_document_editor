    ...
    <voice xml:lang='${item.locale}' xml:gender='${item.gender}' name='${item.value}'>
      ...
      if (error.response) {
        try {
          ...
          toast({
            duration: 2000,
            description: (ErrMessage(errorData.error.err_code, language))
          })
        } catch (parseError) {
          toast({
            duration: 2000,
            description: (ErrMessage(0, language))
          })
        }
      } else {
        toast({
    ...
    return new Promise((resolve, reject) => {
      ...
      fetch(source)
        ...
        .then((audioBuffer) => {
          ...
          // If an object URL is created, it needs to be released
      ...
      <div className='flex justify-between items-center'>
        ...
        </div>
        {
          <div className={`border p-2 rounded-[100%] ${audioData.audioSrc ? 'cursor-pointer' : 'cursor-no-drop'} `} onClick={onDownload}>
            <IoDownloadOutline className={`text-[25px] ${audioData.audioSrc ? 'text-foreground' : 'text-[#888]'}`} />
          </div>
        }
      </div>
    </div>
